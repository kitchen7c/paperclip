  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label={t("Title")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("Capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                ...
                placeholder={t("Describe what this agent can do...")}
            ...
            {isLocal && (
              <>
                <Field label={t("Prompt Template")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          <Field label={t("Adapter type")} hint={help.adapterType}>
            <AdapterTypeDropdown
              value={adapterType}
              onChange={(t) => {
                if (isCreate) {
          ...
          {/* Working directory */}
          {isLocal && (
            <Field label={t("Working directory")} hint={help.cwd}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                Prompt template is replayed on every heartbeat. Prefer small task framing and variables like <code>{"{{ context.* }}"}</code> {t("or")}<code>{"{{ run.* }}"}</code>; avoid repeating stable instructions here.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              <Field label={t("Bootstrap prompt (first run)")} hint={help.bootstrapPrompt}>
                <MarkdownEditor
                  value={
                    isCreate
                      ? val!.bootstrapPrompt
              ...
              <Field label={t("Extra args (comma-separated)")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("e.g. --verbose, --foo=bar")}
                />
              </Field>

              <Field label={t("Environment variables")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("Timeout (sec)")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("Interrupt grace period (sec)")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              />
              <Field label={t("Cooldown (sec)")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("Max concurrent runs")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
const ENABLED_ADAPTER_TYPES = new Set(["claude_local", "codex_local", "gemini_local", "remote_cli", "opencode_local", "cursor"]);
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("Thinking effort")} hint={help.thinkingEffort}>
